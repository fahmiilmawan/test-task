...
export default function ChildIndex({ parent, accounts, filters = {} }) {
    const [search, setSearch] = useState(filters.search || '');

    useEffect(() => {
        const timer = setTimeout(() => {
            if (search !== (filters.search || '')) {
                router.get(route('account.show', parent.id), { search: search }, {
                    preserveState: true,
                    preserveScroll: true,
                    replace: true,
                });
    ...
    return (
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-4">
                    <Link href={route('account.index')} className="text-gray-500 hover:text-gray-700 transition-colors">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Managing Child Accounts</p>
                    </div>
                    <div className="flex justify-end ml-auto">
                        <Link href={route('account.create', { parent_id: parent.id })}>
        ...
        >
            <Head title={`Children of ${parent.name}`} />
